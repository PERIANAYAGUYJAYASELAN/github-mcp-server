...
from perianayaguyjayaselan_demo_api_scalar_galaxy_python.httpclient import HttpClient
from perianayaguyjayaselan_demo_api_scalar_galaxy_python.sdkconfiguration import (
    SDKConfiguration,
)
...
class ClientCredentialsHook(SDKInitHook, BeforeRequestHook, AfterErrorHook):
    ...
    def get_credentials_global(self, security: Any) -> Optional[Credentials]:
        if security is None or security.o_auth2 is None:
            return None

        return Credentials(
            client_id=security.o_auth2.client_id,
            client_secret=security.o_auth2.client_secret,
            token_url=security.o_auth2.token_url,
        )
